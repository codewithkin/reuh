...
export default function Previousdegree ({degreeNumber, updateFunc, degrees}: {degreeNumber: {degreeNumber: number}, degrees: Array<{degreeNumber: number}>, updateFunc: Dispatch<SetStateAction<{degreeNumber: number}>>}) {
    const handleDelete = () => {
        // Remove the last degree from the array
        const newdegrees = degrees.filter((degree) => degree.degreeNumber !== degreeNumber.degreeNumber);
        updateFunc(newdegrees);
    }
